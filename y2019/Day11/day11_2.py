...
class IntComputer:
    def __init__(self, instructions, phase_value):
        ...
        self.white_tiles = {(0,0)}
        self.black_tiles = set()
        self.output=[]
        self.current_dir = (0, -1)
        self.current_point = (0,0)
        self.all_points = {(0,0)}
        self.output_instructions = 0

    def rotate_and_move(self, dir):
        if dir ==0:
            self.current_dir = (-self.current_dir[1], self.current_dir[0])
        else:
            self.current_dir = (self.current_dir[1], -self.current_dir[0])
        self.current_point = (self.current_point[0]+self.current_dir[0], self.current_point[1]+self.current_dir[1])
        self.all_points.add(self.current_point)

    def paint_tile(self, input_color):
        if input_color == 1:
            if self.current_point in self.black_tiles:
                self.black_tiles.remove(self.current_point)
            self.white_tiles.add(self.current_point)
        else:
            if self.current_point in self.white_tiles:
                self.white_tiles.remove(self.current_point)
            self.black_tiles.add(self.current_point)
            ...
            match instruction:
                ...
                case 3:
                    ...
                    if self.phase_value is not None:
                        ...
                        self.instructions[addr] = self.decode_input()
                    self.instruction_pointer += 2
                    continue
                case 4:
                    value = self.decode_value(first_parameter,1)
                    if self.output_instructions % 2:
                        self.rotate_and_move(value)
                    else:
                        self.paint_tile(value)
                    self.output_instructions += 1
                    self.instruction_pointer += 2
                    continue
                case 1:
...
def get_my_answer():
    ...
    white_tiles = list(computer.white_tiles)
    max_x = max(computer.all_points, key=lambda z:z[0])[0]
    max_y = max(computer.all_points, key=lambda z:z[1])[1]
    min_x = min(computer.all_points, key=lambda z:z[0])[0]
    min_y = min(computer.all_points, key=lambda z:z[1])[1]
    mapping = []
