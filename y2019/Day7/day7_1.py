import itertools
import pathlib
from Tools.tools import load_data_as_lines, load_data, load_data_as_int, timeexecution
from aocd import submit

filepath = pathlib.Path(__file__).parent.resolve()
EXAMPLE = False
SUBMIT_ANSWER = False


def int_computer(data, input_values):
    next_instruction = 0
    outputs = []
    while True:
        opcode = str(data[next_instruction])
        opcode = "0" * (5 - len(opcode)) + opcode
        instruction = int(opcode[-2:])
        first, second, third = int(opcode[-3]), int(opcode[-4]), int(opcode[-5])
        match instruction:
            case 99:
                break
            case 3:
                first_val = next_instruction + 1 if first else data[next_instruction + 1]
                data[first_val] = input_values.pop(0)
                next_instruction += 2
                continue
            case 4:
                first_val = next_instruction + 1 if first else data[next_instruction + 1]
                outputs.append(int(data[first_val]))
                next_instruction += 2
                continue
            case 1:
                first_val = data[next_instruction + 1] if first else data[data[next_instruction + 1]]
                second_val = data[next_instruction + 2] if second else data[data[next_instruction + 2]]
                third_val = next_instruction + 3 if third else data[next_instruction + 3]
                data[third_val] = first_val + second_val
                next_instruction += 4
            case 2:
                first_val = data[next_instruction + 1] if first else data[data[next_instruction + 1]]
                second_val = data[next_instruction + 2] if second else data[data[next_instruction + 2]]
                third_val = next_instruction + 3 if third else data[next_instruction + 3]
                data[third_val] = first_val * second_val
                next_instruction += 4
            case 5:
                first_val = data[next_instruction + 1] if first else data[data[next_instruction + 1]]
                second_val = data[next_instruction + 2] if second else data[data[next_instruction + 2]]
                if first_val:
                    next_instruction = second_val
                else:
                    next_instruction += 3
            case 6:
                first_val = data[next_instruction + 1] if first else data[data[next_instruction + 1]]
                second_val = data[next_instruction + 2] if second else data[data[next_instruction + 2]]
                if not first_val:
                    next_instruction = second_val
                else:
                    next_instruction += 3
            case 7:
                first_val = data[next_instruction + 1] if first else data[data[next_instruction + 1]]
                second_val = data[next_instruction + 2] if second else data[data[next_instruction + 2]]
                third_val = next_instruction + 3 if third else data[next_instruction + 3]
                data[third_val] = int(first_val < second_val)
                next_instruction += 4
            case 8:
                first_val = data[next_instruction + 1] if first else data[data[next_instruction + 1]]
                second_val = data[next_instruction + 2] if second else data[data[next_instruction + 2]]
                third_val = next_instruction + 3 if third else data[next_instruction + 3]
                data[third_val] = int(first_val == second_val)
                next_instruction += 4
    return outputs, data


def get_my_answer():
    data = list(map(int, load_data(filepath, example=EXAMPLE).split(",")))
    possible_numbers = [0,1,2,3,4]
    all_combinations = itertools.permutations(possible_numbers, 5)
    all_outputs = []
    for combination in all_combinations:
        outputs = [0]
        for i in range(0, 5):
            outputs, data = int_computer(data, [combination[i], outputs[-1]])
        all_outputs.append((combination, outputs[0]))
    print(all_outputs)
    return max(all_outputs, key=lambda x: x[1])[1]


@timeexecution
def execution():
    submit_answer = SUBMIT_ANSWER
    my_answer = get_my_answer()
    print(my_answer)
    this_day = int(str(filepath).split("\\")[-1][3:])
    this_year = int(str(filepath).split("\\")[-2][1:])
    if submit_answer:
        submit(my_answer, part="a", day=this_day, year=this_year)
